...
func (bm BtMgmt) ReadConnectionList(controllerID uint16) (res *ConnectionInfoList, err error) {
	payload, err := globalMgmtConn.RunCmd(controllerID, CMD_GET_CONECTIONS)
	if err != nil {
		return
	}
	res = &ConnectionInfoList{}
...
func (bm BtMgmt) SetDiscoverable(controllerID uint16, discoverable Discoverability, timeoutSeconds uint16) (currentSettings *ControllerSettings, err error) {
	...
	if discoverable == NOT_DISCOVERABLE {
		timeoutSeconds = 0
	} // Could also be handle via invalid parameters error
...
func (bm BtMgmt) SetSecureSimplePairing(controllerID uint16, secureSimplePairing bool) (currentSettings *ControllerSettings, err error) {
	...
	if secureSimplePairing {
		bSecureSimplePairing = 1
	}
